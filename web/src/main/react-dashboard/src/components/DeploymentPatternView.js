    ...
    render() {
        ...
        this.state.hits.map((value, index) => {
            if (groupByDeployment[value.lastBuild.deploymentPattern] === undefined) {
                groupByDeployment[value.lastBuild.deploymentPattern] = [{ 'lastBuild': value.lastBuild, 'lastFailed':
                    value.lastFailure }]
            } else {
                groupByDeployment[value.lastBuild.deploymentPattern].push({ 'lastBuild': value.lastBuild, 'lastFailed':
                    value.lastFailure })
            }
        });
        return (
