...
openssl s_client -connect <server-ip-or-hostname>:443 < /dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > server.crt
$JAVA_HOME/bin/keytool -import -alias testgrid-live.private.wso2.com -keystore $JAVA_HOME/jre/lib/security/cacerts -file server.crt

sudo mkdir /testgrid
sudo chown ubuntu:ubuntu /testgrid
mkdir /testgrid/jenkins-home
wget https://<jenkins-master-url>/jnlpJars/slave.jar --no-check-certificate
