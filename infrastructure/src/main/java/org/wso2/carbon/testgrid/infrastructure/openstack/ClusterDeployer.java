...
//        executeCommand(DeployerConstants.COMMAND_DEPLOY_ARTIFACTS);
    ...
    public static void undeployArtifacts (){
        try {
            String target = new String(DeployerConstants.KUBERNETES_HOME + "/wso2is/undeploy.sh");
    ...
    public static void deployArtifacts (){
        try {
            String target = new String(DeployerConstants.KUBERNETES_HOME + "/wso2is/deploy.sh");
            Runtime rt = Runtime.getRuntime();
            Process proc = rt.exec(target);
            proc.waitFor();
            StringBuffer output = new StringBuffer();
