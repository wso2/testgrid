...
/**
 * This is the factory class which is used to get Infrastructure Providers.
 */
public class InfrastructureProviderFactory {
    ...
    public static InfrastructureProvider getInfrastructureProvider(String type) throws UnsupportedProviderException,
            InfrastructureProviderInitializationException {

        for (InfrastructureProvider provider : providers) {
            if (type.equalsIgnoreCase(provider.getProviderName())) {
                try {
                    ...
                    throw new InfrastructureProviderInitializationException("Exception occurred while instantiating the" +
                            " InfrastructureProvider for requested type '" + type + "'", e);
        ...
        throw new UnsupportedProviderException("Unable to find a InfrastructureProvider for requested type '" + type + "'");
    }
}
